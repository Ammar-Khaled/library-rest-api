...
router.post('/', (req, res) => {
  ...
  const author = db.prepare('SELECT * FROM authors WHERE id = ?').get(info.lastInsertRowid);
...
router.patch('/:id', (req, res) => {
  ...
  db.prepare('UPDATE authors SET name = ?, bio = ? WHERE id = ?').run(newName, newBio, req.params.id);
...
router.delete('/:id', (req, res) => {
  const info = db.prepare('DELETE FROM authors WHERE id = ?').run(req.params.id);
  if (info.changes === 0) {
    return res.status(404).json({ error: 'Author not found' });
  }
  res.status(204).end();
